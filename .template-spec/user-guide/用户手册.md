...
3. 根 `CONTEXT.md`、`AGENTS.md` 和 `.template-spec/user-guide/用户手册索引.md` 可读。
...
已有 Spec、设计、冻结接口或后端交付包时，按[阶段接入与产物复用使用手册](阶段接入与产物复用使用手册.md)从最近可信阶段继续。手册提供可复制指令、前端接收操作、命令及阻塞处理，帮助复用有效上游资产并补齐当前阶段缺口。
