...
`resolveBackendPlatform(binding, { root })` 在未显式传入 catalog 时读取该 `root` 下的 `.template-spec/engineering/backend-platforms.json`；未传 `root` 时读取模板仓目录。这样 Slice freshness 与隔离 fixture 使用各自的目录事实，不会意外回退到模板工作区。
