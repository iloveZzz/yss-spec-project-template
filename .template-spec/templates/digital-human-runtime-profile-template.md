...
> 从 `.template-spec/agents/digital-human-roles.yaml` 复制 title / description / skill。不要手写第二套职责。先选 `runtime_id`，再按该运行时适配器落地。
...
- 会签按 YAML `gate_policy` 写 `.template-spec/templates/approval-record-template.yaml`，填 `runtime_id` 与 `principal_ref`。恢复前跑 `scripts/verify-approval-record`。
- 权威产物写 git。
