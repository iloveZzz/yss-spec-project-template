...
| API Contract Agent | OpenAPI Draft / Freeze | OpenAPI YAML、契约测试建议 |
...
- API Contract Agent 和 Architecture Agent 分别审查 Draft 与 DDD 边界，前提是校准后的 PRD 已稳定，且 UI 功能的页面 / 原型 / 交互设计已评审或明确无 UI 影响。
- 多个独立垂直切片，且不会修改同一批文件。
...
| 阶段依据 | 只有产品想法，缺少 Discovery、PRD 初稿、页面 / 原型 / 交互设计、PRD 校准和 OpenAPI |
| 已有资产 | 初始想法、目标用户假设 |
| 缺失资产 | 竞品矩阵、Discovery、PRD 初稿 / 需求基线、页面 / 原型 / 交互设计、PRD 校准 / 需求冻结、OpenAPI Draft、工程基线、架构设计、设计审查、垂直切片 |
| 是否阻塞 | 阻塞实现，不阻塞产品探索 |
...
PRD 把追问结论变成可交付需求，不写实现代码。这里的 PRD 先形成“需求基线”，不等于最终冻结版本；有用户界面的功能，还需要经过页面 / 原型 / 交互设计校准后，再回填 PRD 并进入 OpenAPI Draft。
...
- [ ] 测试 seam 已明确。
- [ ] 待人工确认问题没有伪装成已确认需求。
- [ ] 有用户界面的功能已明确需要进入页面 / 原型 / 交互设计；简单文案或纯后端能力可记录跳过原因。
...
-> 交互发现遗漏后回填 PRD
-> PRD 与交互设计一起作为 OpenAPI Draft 输入
```
...
```text
使用 yss-product-lifecycle，基于校准后的 PRD、页面 / 原型 / 交互设计和 OpenAPI Draft 检查“数据中台数据建模 MVP”的工程基线。
...
## 11. 阶段 8：OpenSpec / Comet design 与设计审查
...
请复用校准后的 PRD、页面 / 原型 / 交互设计、OpenAPI Draft 和工程基线，重点完成行为规格、状态流、风险、测试 seam 和设计审查清单。
```
...
- [ ] 阻断项已回到对应阶段修正。
- [ ] 页面 / 原型 / 交互设计已通过评审，并已反推 OpenAPI Draft。
- [ ] OpenAPI Draft 已通过设计审查，可以进入 Freeze。
- [ ] OpenSpec / Comet change 目标单一，不混入指标管理、血缘分析等额外目标。

## 12. 阶段 9：OpenAPI Freeze
...
```text
基于 PRD、已评审交互设计、冻结 OpenAPI 和架构设计，
...
```text
使用 yss-router，基于“数据建模 Slice 4：发布前校验”的 PRD、交互设计、冻结 OpenAPI 和架构设计，
...
## 15. 阶段 12：独立审查与 fresh verification
...
- [ ] 页面 / 原型 / 交互设计已评审，并能覆盖 P0 用户故事、状态和异常路径。
- [ ] 交互设计发现的需求缺口已回填 PRD，PRD 和设计资产已互相链接。
- [ ] OpenAPI Draft 已完成；涉及 API 时已通过设计审查并 Freeze。
- [ ] 工程基线和 YSS DDD 分层已确认。
