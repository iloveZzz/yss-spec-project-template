...
grill-with-docs
  -> to-prd
  -> OpenAPI Draft
  -> OpenSpec / Architecture validation
  -> OpenAPI Freeze
  -> to-issues
  -> implement with tdd
  -> review / verify
...
- 先在 `docs/api/specs/*.yaml` 形成 Draft。
- 以 Draft 形式接受架构 / OpenSpec 反馈。
- 开发前冻结为 OpenAPI Freeze。
- 再实现前后端并补充测试。

OpenSpec 的 `specs/.../spec.md` 更偏“行为规格”，OpenAPI YAML 更偏“接口契约”。两者可以互相引用，但职责不同：行为规格改变状态流、错误场景或权限边界时，必须回写 OpenAPI Draft 并重新 Freeze。
