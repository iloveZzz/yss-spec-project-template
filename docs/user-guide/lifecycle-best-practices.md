...
只要涉及前后端接口，就先形成 API 影响记录和契约草案；必要时在 `docs/api/specs/*.yaml` 产出 review-only OpenAPI Draft。Draft 经过工程基线、系统/数据架构、OpenSpec / Comet design 和设计审查反审后，才能冻结为 OpenAPI Freeze，再实现前后端或生成客户端。
...
Draft 阶段允许调整接口，但不能被当成稳定实现契约；Freeze 之后的契约变更必须回到工程基线、架构 / OpenSpec / Comet design 和设计审查点，不能边写代码边悄悄改接口。
...
-> PRD
-> API impact / contract draft
-> Architecture / ADR / OpenSpec / Comet design
-> OpenAPI Freeze
-> OpenSpec change
-> Issues
-> Release Note
...
```text
基于 PRD、冻结 OpenAPI 和 OpenSpec / 架构设计，把功能拆成 3-6 个可独立验收的垂直切片。
