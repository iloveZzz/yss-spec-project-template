...
| 层级 | 放什么 | 不放什么 |
|---|---|---|
| `AGENTS.md` | Agent 启动即必须遵守的工程红线、技能入口和阶段门禁 | 完整代码手册、临时实现细节 |
...
### 1.4 一个垂直切片 Issue 只解决一个目标

垂直切片 Issue 应该围绕一个清晰目标。
...
如果一个 Issue 需要多天都说不清完成标准，通常应该拆分。
...
进入 PRD 前先完成机会探索环。它不是固定的“头脑风暴 → 发现”单向步骤，而是把事实、假设和 MVP 边界来回校准：
...
- PRD 前已用 `grill-with-docs` 澄清关键边界，或已有等价的确认记录。
- 有 PRD。
...
| 页面状态、交互、字段展示 | 产品设计 / 原型评审 | 交互说明、状态矩阵、PRD 回填项 |
| 验收标准、业务规则、非目标边界 | PRD 校准 | PRD 更新、产品总体设计影响 |
...
只有当小迭代推翻了用户、痛点、MVP、非目标或产品边界，才回到 Discovery / 业务架构 / PRD 初稿阶段。
...
如果 bug 修复引出新业务规则，升级到 PRD 校准、产品总体设计、OpenAPI 或系统概要设计等最早受影响阶段。
...
- [ ] 如果触碰权限、认证、SQL、迁移、加密或公共 API，已完成人工确认或标记 `TODO-HUMAN-REVIEW`。
- [ ] 没有调试代码。
- [ ] 任务清单已更新。
...
- [ ] 垂直切片任务全部完成或明确取消。
- [ ] 必要的 Spec Delta 已与 PRD / OpenAPI / Issue 互链。
- [ ] 验证结果已记录。
- [ ] 经验已沉淀到 `AGENTS.md`、`CONTEXT.md` 或 ADR。

使用 Issue 追踪时，发布前应确认所有垂直切片任务已完成或明确取消，并记录验证与回滚结论。
...
| 只在聊天里讲需求 | 后续无法追溯，AI 容易遗忘 | 写入 PRD、Spec Delta 或 Issue |
| 先写页面再补接口 | 字段和错误结构反复返工 | 先写 OpenAPI |
| 按层拆任务 | 做了很多代码但无法演示 | 按垂直切片拆 |
| 不经 yss-router 随手选技能 | 规范加载过多或漏掉关键约束 | 先选最小技能集 |
| 把完整编码手册塞进 AGENTS.md | 入口指令变长且容易和 YSS skills 冲突 | AGENTS.md 放红线，细则放 skills / docs |
...
| 阶段产物不 checkpoint | PRD / Spec Delta / OpenAPI / Issue 决策难追溯 | 阶段结束做 Issue 同步和 Git checkpoint |
...
```text
基于 OpenAPI Draft、工程基线、架构设计和 Issue 行为规格，
...
```text
基于 PRD、冻结 OpenAPI 和架构设计，把功能拆成 3-6 个可独立验收的垂直切片。
...
```text
按当前 垂直切片 Issue 的 tasks 推进下一个未完成任务。
