...
| OpenAPI client 刷新 | `yss-openapi-governance` 从冻结 YAML 派生 JSON，随后 `api-integration` 运行 Orval；只在 Freeze 后或按实现契约刷新时使用 |
...
目标：选择最小 YSS 技能集。
请说明每个候选技能需要或不需要的理由，尤其是 yss-domain、yss-repository、yss-mybatis、yss-web-controller、yss-ui、yss-openapi-governance、api-integration。
