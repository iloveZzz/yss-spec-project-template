...
| 本模板层级 | 默认能力 | 说明 |
|---|---|---|
| 需求与任务 | `competitive-intelligence`、`research`、`grill-with-docs`、`prototype`、`to-prd`、`to-issues` | 收集竞品 / 市场 / 技术事实、澄清需求、用一次性原型回答设计问题、沉淀 PRD、拆端到端垂直切片 |
...
| 使用 skills | `competitive-intelligence`（需要竞品 / 市场事实时）、`research`（需要技术 / 标准 / API 一手资料时）、`grill-with-docs`、`prototype`（需要一次性验证时）、`domain-modeling`、`to-prd`、`to-issues`、`implement`、`tdd`、`code-review` |
...
- 技术 / 标准 / 第三方 API 事实影响 PRD、OpenAPI、架构或验收时，必须有 `research` 或等价一手资料记录，并在下游资产中引用。
- 一次性 `prototype` 只回答设计问题；结论必须回填到 PRD、设计、ADR 或 Issue，原型代码不得作为生产实现或 YSS 高保真 HTML 原型交付。
...
| `prototype` | 状态机、复杂规则或 UI 方向需要可运行反馈 | 生成一次性逻辑 / UI 原型并沉淀结论 | PRD、设计、ADR 或 Issue 的结论章节 |
...
| 一次性原型结论 | PRD、设计、ADR 或垂直切片 Issue |
| 跨线程 / 跨仓库交接 | Issue / MR / PR 评论或 `docs/implementation/` |
