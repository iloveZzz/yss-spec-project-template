...
| 2. PRD -> 产品总体设计 -> 原型校准 | 3. 业务 / PRD / 功能架构；4. 产品设计与需求冻结 | 4. 需求基线 / 功能架构；5. 页面 / 原型 / 交互设计；6. 原型评审；7. PRD 校准 / 需求冻结 | PRD、产品总体设计 / 功能架构、低保真原型、页面清单、状态矩阵、原型评审、Ant Design v6 高保真 HTML 原型、AntD CLI 校验证据、用户确认记录、需求冻结 | `yss-design-system`、`product-design:index`、Product Design focused skills（`$get-context` / `$ideate` / `$prototype` / `$image-to-code` / `$url-to-code`）、`antd`、`prototype-review` | 无 UI 可省略后续交互原型；进入 PRD 初稿 / 需求基线流程时不得省略产品总体设计及其低保真原型；有 UI 时产品原型产出以 `product-design:index` 路由为主，低保真评审通过后不得省略高保真 HTML 原型，可由系统 / Agent 自动产出，产出前后必须使用 `antd` CLI 校验并在产出后获得用户确认；中等变更只补受影响页面 | 半自动检查 PRD 字段、产品总体设计、低保真原型、状态矩阵、原型评审、高保真 HTML 原型、AntD CLI 校验证据和用户确认记录 |
...
| 6. 垂直切片 Issue | 7. 垂直切片与 TDD 实现 | 15. 实施计划 | 垂直切片 Issue、实现路由、Build Architecture Checklist | `to-issues`、`yss-router`、`implement` | 小改动可用单一 Issue / 实施备注替代 | 半自动检查切片是否端到端而非横向拆层 |
...
- 13 个工作单元不能替代 OpenAPI Freeze、Issue formalization、Build Architecture Checklist、安全红线和 fresh verification。
