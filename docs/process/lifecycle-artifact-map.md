...
本文是 `project-instance` 产品研发流程中主阶段、门禁、产物和退出标准的权威索引。日常执行使用 8 个主阶段；21 个门禁 / 职责点全部按影响面条件强制。未命中触发条件时记录 `not-applicable` 及原因，不生成空文档。`template-source` 使用 `AGENTS.md` 定义的模板维护流程，不适用本表的产品产物链。

项目实例的默认流程为 Matt Pocock Engineering Skills + YSS + OpenAPI：用 Matt skills 做需求澄清、Spec、Ticket 拆分、实现、TDD、诊断、审查和架构治理；用 YSS skills 做业务工程规范；用 OpenAPI 3.1 做前后端契约。Spec Delta 只表达相对既有冻结 Spec 基线的高风险行为差异，不用于全新产品或全新模块，也不恢复 OpenSpec CLI 或额外变更状态机。
...
|---|---|---|---|
| 1. 入口分诊 | Explorer 检查现有资产、Ticket、git 状态 | 生命周期阶段判定、最近可信阶段选择 | 分诊记录或 checkpoint 说明是否使用 subagent |
...
| 6. 契约冻结与 Ticket 正式化 | 垂直切片草案、实现仓库 / 脚手架状态检查 | Ticket 范围最终裁剪、Ticket tracker 状态同步 | Ticket 或 implementation routing 记录 subagent 分工 |
...
| 13. 契约冻结 | 6. 契约冻结与 Ticket 正式化 | OpenAPI Freeze 记录或无 API 影响记录 | 有 API 影响时必需；无 API 时记录结论 |
| 14. Ticket 正式化 | 6. 契约冻结与 Ticket 正式化 | 垂直切片 Ticket、`to-tickets` 输出或等价任务记录，并记录受影响前后端工程存在性判定、`scaffold_status` 与外部脚手架目标确认 | 正式垂直切片前必需 |
| 15. 实施计划 | 7. 垂直切片与 TDD 实现 | 垂直切片 Ticket、批准的 Slice Implementation Contract、实施计划、Build Architecture Checklist、回滚点，以及缺失前后端工程时的脚手架初始化记录；涉及 DDL / SQL / 数据库迁移、权限接入或审计日志时记录人工确认结论 | 正式开发前必需 |
...
| Spec | `docs/requirements/<feature>-spec.md` | `docs/templates/spec-template.md` |
...
| 垂直切片 Ticket | GitLab / GitHub Issues、local `tickets.md` 或 `docs/requirements/tickets/<feature>-slice.md` | `docs/templates/vertical-slice-ticket-template.md` |
| 实现路由 | `docs/requirements/tickets/<feature>-implementation-routing.md` | `docs/templates/implementation-routing-template.md` |
...
| 审查报告 | MR / PR 评论或 `docs/requirements/tickets/<feature>-review.md` | `docs/templates/review-report-template.md` |
...
| Subagent 任务包 | Ticket / 阶段文档或 `docs/requirements/tickets/<feature>-subagent-task-package.md` | `docs/templates/subagent-task-package-template.md` |
...
- `route` 与 `audit` 必须只读；`orchestrate` 与 `resume` 采用有界自动推进，遇到人工门禁、真实阻塞、新授权或完成裁决时暂停。
- Matt 五态标签与资产/门禁状态必须分层表达；只有解除阻塞且全部必要门禁通过的垂直切片 Ticket 才能使用 `ready-for-agent`。
- 上游资产变化后按影响类型将实际受影响的下游标记为 `stale`；不得只凭父 Ticket 阶段或文件存在继续实现，也不得默认重跑全部阶段。
