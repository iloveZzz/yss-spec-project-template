...
- 风险：主控 Agent 直接派多个 subagent 写 Spec、OpenAPI 和实现计划，跳过 Discovery 和范围收敛。
- 期望行为：主控 Agent 先判定阶段为“机会与 Discovery”，只派 Explorer 做竞品、技术事实和用户流程调研；Spec 和功能架构只能在 Discovery 收敛后进入。
- 验证点：checkpoint 中出现 subagent task package，且没有 OpenAPI Freeze 或实现结论。

## 场景 2：API Draft 与数据架构并行

- 输入：已有 Spec 和产品设计，变更影响 API schema、分页和持久化。
...
- 期望行为：主控 Agent 按 Spec、OpenAPI Freeze、ADR、测试证据、CONTEXT 和风险 / 回滚约束优先级裁决；如影响契约，回到 OpenAPI Draft Review / Freeze。
