...
`docs/agents/issue-tracker.md` 的 `tracker.lifecycle_tracking_version: 1` 表示项目已启用。新初始化默认启用；attach / sync 保留已有配置，未声明的旧项目不自动启用。只读检查不修改项目；旧项目先 plan 再显式 apply。模板源只维护机制，不在模板根生成产品工作项。
