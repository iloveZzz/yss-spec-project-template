...
| G3 | 产物未绑定 pipeline | guard 查 `docs/api/specs/*` 任意 yaml、`.hermes/plans/*` 任意 md | A 的 spec 能让 B 过门禁 |
...
meta:
  version: "1.1"
  template: "hermes-project-template"
...
pipelines:
  user-login:
    ...
    artifacts:                   # 产物挂载点：驱动器读写、guard 按此检查
      spec:    docs/api/specs/user-login.yaml
      plan:    .hermes/plans/user-login.md
...
| design | plan | `.hermes/plans/<pipeline>.md` | Plan Agent |
