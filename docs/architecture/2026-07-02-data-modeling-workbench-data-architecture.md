...
OpenAPI 当前保留 `reviewVersion` 字段，可继续作为并发控制字段。Design Review 可确认是否在 OpenAPI Freeze 前明确“一次提交一个 reviewId”的产品规则。
...
当前 OpenAPI Draft 已覆盖主要数据模型。数据架构阶段建议在 Freeze 前确认以下细节：

| 项 | 建议 | 是否阻断 Design Review |
|---|---|---|
| 发布 DDL 确认 | `CreateVersionRequest.ddlDraftAcknowledged` 当前有 `const: true`，建议加入 required，确保发布请求显式确认 DDL 仅草案。 | 不阻断 Design Review，阻断 OpenAPI Freeze。 |
| 幂等键 | `X-Idempotency-Key` 当前参数为 optional，建议关键命令在描述或契约中明确 required：apply import、submit review、decision、publish、create export、retry export。 | 不阻断 Design Review，阻断实现前契约测试。 |
| 快照校验值 | `ModelVersionSummary/Detail` 可考虑补充 `snapshotChecksum` 或 `snapshotSchemaVersion`，便于追溯和兼容。 | 建议项。 |
| 评审再次提交策略 | 建议确认“再次提交生成新 reviewId”并写入 PRD/架构或 OpenAPI 描述。 | 不阻断 Design Review，OpenAPI Freeze 前确认。 |
...
| 评审再次提交生成新 reviewId | 中 | 历史清晰，评审列表更多。 | Design Review 前确认。 |
...
5. OpenAPI Freeze 前建议项已登记，尤其是 DDL 草案确认、关键命令幂等键、评审再次提交策略。
...
- 待确认：版本号规则、导出过期时间、导入预览保留期、评审再次提交策略、规则集模板来源。
