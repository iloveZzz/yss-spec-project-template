...
LLM Wiki 是由 `raw/`、`wiki/` 与 `.wiki-manifest.json` 组成的本地持久知识库。它是中间表示（IR），live 源才是事实；它不是 `research` 一次性笔记，也不替代权威源。
...
一次性一手资料走 `research`；要把研究结果落成持久 wiki 走 `llm-wiki`。`/deep-research` 若产出是 wiki 则路由 `llm-wiki`。`understand-knowledge` 是可选外部配对，用于图看板，不是本模板必装技能，也不是本 skill 的捆绑依赖。强制入口见 [[Agent入口规则]]；`research` 本身属于 [[Matt技能体系]]。
