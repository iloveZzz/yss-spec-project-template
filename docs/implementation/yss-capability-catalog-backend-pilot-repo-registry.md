...
> 本登记由 `implementation-repo-onboarding` 只读扫描产生。它只把现有实现仓库作为 Pilot 候选登记到 Harness，不修改、提交或推送 `modeling-yss`。
...
| default_branch | `dev` |
| local_worktree | `/Users/zhudaoming/Projects/modeling-yss` |
...
| scaffold_target_confirmed | 否；现有本地实现位置已存在，但 Pilot 目标分支和干净基线尚未确认 |
...
- 仓库根目录缺少 `yss-project.yaml`。不能根据目录、Git remote 或产品文档推断仓库身份；在身份清单补齐并通过迁移检查前，不能将它作为符合 Harness 身份契约的 fresh `project-instance`。
- 当前 `dev` 工作区有 14 个已修改文件、23 个未跟踪文件，共 37 个 Git 状态项。该工作区不能作为 Pilot 的干净基线，也不能覆盖、清理或回滚这些用户变更。
...
| 当前选择状态 | `candidate-not-approved`；须先完成身份、干净分支、实现仓库边界和人工风险确认 |
| 首轮 Pilot 约束 | 只验证路由、合同、TDD seam 和测试证据；不执行生产 migration，不接真实 IAM / SSO，不接生产审计 adapter，不改变外部仓库代码 |
...
| test_command | `cd apps/backend/asset-product-modeling-service && ./mvnw test -DfailIfNoTests=false` | PASS；127 tests，0 failures / errors / skipped；结果来自当前 dirty worktree，不作为可合并证据 |
| build_command | `cd apps/backend/asset-product-modeling-service && ./mvnw -q -DskipTests validate` | PASS；只证明当前工作树可完成 Maven validate |
...
| yss_backend_baseline | 部分接入 | 多模块 DDD 工程、Backend Slice Contract、YSS DTO / Web / Repository 纠偏记录 | 根仓库身份清单、根目录 wrapper、owner、远端 CI 和干净基线未完成 |
...
|---|---|---|---|
| 根目录缺少 `yss-project.yaml` | 无法按仓库身份契约确认 project-instance；可能把旧项目文档误当模板事实 | owner 在独立变更中补齐并执行迁移检查；不在本次 onboarding 擅自写入 | 是 |
| `dev` 工作区 37 个 dirty 状态项 | 无法区分本 Pilot 证据与用户正在进行的产品 / 原型修改；存在覆盖风险 | owner 创建干净隔离分支 / worktree，并记录基线 commit；不得清理现有变更 | 是 |
...
| 仓库身份与实现边界 | 是 | Harness owner / implementation repo owner | 当前阻断；需确认 `project-instance` 身份、隔离分支和是否保留本地实现位置例外 | `yss-project.yaml`、implementation routing、repo registry |
...
- 当前阶段：实现仓库登记 / Pilot readiness，状态 `candidate-blocked`。
- fresh verification（本轮只读）：
  - `cd /Users/zhudaoming/Projects/modeling-yss/apps/backend/asset-product-modeling-service && ./mvnw -q -DskipTests validate`：PASS。
  - `cd /Users/zhudaoming/Projects/modeling-yss/apps/backend/asset-product-modeling-service && ./mvnw test -DfailIfNoTests=false`：PASS，127 tests，0 failures / errors / skipped。
- 这些命令验证的是当前 dirty worktree 的现状，不构成 Pilot 放行；完成身份、干净 checkpoint、合同重编译和独立审查前，不得进入业务写入。

## 8. 下一步门禁

1. 人工确认是否把 `modeling-yss` 作为 Issue #41 的外部 Pilot implementation repo。
2. 在不覆盖现有 37 个 dirty 状态项的前提下，建立干净 worktree / 分支并记录基线 commit。
3. 补齐或确认根目录 `yss-project.yaml`，完成 project-instance 身份迁移检查。
4. 用当前 catalog 的 `backend-vertical-slice` profile 重新路由 VS-001，替换历史 retired skill 名称，并由生命周期编排器批准新的 Slice Implementation Contract。
5. 确认首轮不执行生产 migration、不接真实 IAM / SSO、不接生产审计 adapter，并将其写入 Build Architecture Checklist 与 `seam_deferred`。
6. 在干净基线上重新执行模板验证、Router / lifecycle 场景、后端 `./mvnw` 验证和独立 review；通过后再创建 `ready-for-agent` 垂直切片 Ticket。
