...
|---|---|---|---|---|---|---|---|
| backend | 是 / 否 / 不适用 | existing / required / initialized / not-applicable | 是 / 否 / 不适用 | 复用现有 / 初始化 / 阻塞 | `yss-ddd-scaffold-generator` / `yss-layered-mvc-scaffold-generator` / none |  | 新建填写 gate、architecture_family、精确 Spring Boot 版本、decision_id 与 digest；既有工程填写登记值复用证据 |
...
当 backend `scaffold_status=required` 时，生命周期先按领域复杂度推荐 DDD/MVC，并展示平台清单中的精确 Spring Boot / Java 版本，以批量决策表取得同一次用户确认并通过 `gate.backend-architecture-platform-approved`。选择写入 `scaffold-architecture-decisions.yaml`；缺失、未确认或 stale 时阻断。门禁通过后才进入对应的 DDD 战术设计或 MVC 技术设计，再登记 `controlled-generation` 工作单元。`domain-driven` 使用 `primary_skill=yss-ddd-scaffold-generator`，`layered-mvc` 使用 `primary_skill=yss-layered-mvc-scaffold-generator`。生成后执行对应基线校验和 `yss-implementation-contract-compiler` 合同重编译。`existing` / `initialized` 核验并复用当前登记架构与平台，不重复询问或生成；架构转换、平台升级另立迁移工作单元。

| 项 | 内容 |
|---|---|
| 前置 | `prototype_confirmation`、工程基线、实现仓库和脚手架目标已确认；`gate.backend-architecture-platform-approved` 已同时确认 DDD / MVC 与精确 Spring Boot 版本，`scaffold-architecture-decisions.yaml` digest 当前；实现合同编译器 schema v4 脚手架合同 draft 已经生命周期批准并持久化为结构化 JSON，生成器通过 `--contract-file` 消费 |
