...
| task_id | 数字人角色 | subagent 角色 | 责任范围 | 允许修改文件 / 模块 | 禁止事项 | 输出回填位置 |
|---|---|---|---|---|---|---|
|  | role.frontend-engineer | Explorer / Drafter / Worker / Reviewer / Verifier | frontend / backend / test / review / verify / docs |  | 不得 Freeze / 不得安全放行 / 不得覆盖他人改动 / 不得会签自己的草稿 |  |
