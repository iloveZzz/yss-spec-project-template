...
In YSS projects, this skill is a visualization and review aid. It can clarify lifecycle artifacts, but it must not replace Discovery, PRD, OpenAPI, ADR, OpenSpec/Comet design, or tests.
...
| Functional architecture | PRD baseline / product design / PRD calibration | feature map, module map, dependency graph, page map | `docs/design/diagrams/` or `docs/architecture/diagrams/` |
| System architecture | Engineering baseline / Architecture / OpenSpec / Comet design | C4/container view, deployment, sequence, DDD boundary, DFD | `docs/architecture/diagrams/` |
...
- If a diagram exposes a missing field, unclear state, dependency, or boundary problem, write that finding back to the source PRD, OpenAPI Draft, architecture document, ADR, OpenSpec/Comet design, or issue.
