...
try {
  ...
  for (const projection of [".agents", ".codex", ".cursor", ".pi"]) {
  ...
  for (const forbidden of [".agents", ".codex", ".cursor", ".pi"]) if (await stat(path.join(root, forbidden)).then(() => true).catch(() => false)) throw new Error(`项目不应携带技能投影目录: ${forbidden}`);
