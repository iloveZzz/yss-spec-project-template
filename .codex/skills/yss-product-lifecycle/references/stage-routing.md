...
| 4. Product design and requirement freeze | 有 UI、页面、交互、状态矩阵、权限状态、空/错/加载态 | PRD, design system, existing UI, component docs | product overview design, interaction spec, state matrix, prototype review, requirement freeze | contract and architecture review |
...
- Treat lifecycle opportunity exploration as product/MVP boundary work. Discovery may produce competitive analysis via `competitive-intelligence`, an opportunity statement, product capability guidance, and downstream impact signals, but it must not freeze PRD, final functional architecture, OpenAPI, data architecture, or technical system design.
- Small iterations only expand from the earliest affected artifact. Do not rerun Discovery or PRD if a copy/style/config change has no API, state, permission, data, module boundary, or security impact.
...
- API changes route through OpenAPI Draft Review and Freeze, then `to-issues`.
...
- If multiple PRDs or issues look relevant, ask the user which one to continue before planning work.

## Required Inspection Before Implementation

1. Inspect the relevant PRD, issue, OpenAPI Freeze or no-API-impact record, and architecture/design review when applicable.
...
请先检查现有 Discovery、PRD、设计、OpenAPI、架构和 Issue 资产，
...
竞品矩阵落点、PRD 输入和下一步 grill-with-docs / to-prd 衔接方式。
```
...
请判断是否需要 OpenAPI Draft、Draft Review、Freeze、to-issues，
...
若 PRD、OpenAPI Freeze 或架构清单缺失，请路由回对应阶段；
