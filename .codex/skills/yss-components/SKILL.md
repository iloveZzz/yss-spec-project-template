...
本技能只负责页面组件、布局和交互呈现规范。请求、分页参数、接口映射交给 `yss-hook` 或 `yss-api-integration`；新建、改造和编排完整业务页面统一交给 `yss-ui-business-page-generation`；表格/树高度细节交给 `yss-use-table-height` 或 `yss-use-tree-height`。
