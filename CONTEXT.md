...
| 研发管理仓库 | 承载 Spec、OpenAPI、架构、Ticket、验证、发布和复盘等研发管理资产的仓库。 | — | 口语「研发仓」指本术语，不要与实现仓库混用。不等同于前端 / 后端代码 monorepo。一体仓只表示与实现项目同居同一 Git 历史，不把两种角色合成一个术语。 |
| 实现仓库 | 承载前端、后端或其他运行时代码及其 Git、CI、MR / PR、测试命令和发布流水线的仓库。 | — | 不要把实现仓库的源码所有权混入研发管理角色；一体仓中源码仍按实现项目登记。 |
| 分仓接入 | 研发管理仓库与前端、后端实现仓库各有独立 Git 历史，靠实现仓库登记和跨仓库切片绑定。 | — | 默认拓扑。口语「分层接入」按本术语理解，不是第四种拓扑。不用 git submodule 或 git subtree 把实现仓嵌进研发管理仓库工作树。不是把源码复制进模板源仓库。不要与一体仓混用。 |
| 一体仓 | 同一 Git 仓库同时承载研发管理资产与前端、后端运行时代码。 | — | 只用于某个 `project-instance`，不是模板源仓库。口语 all in one / all-in-one 不用作规范名；也不要用 GitLab 文档里的 monorepo 单独代替本术语。 |
| 跨仓库契约变更 | 需要两个或多个独立 Git 仓库协同实现、验证和按顺序发布的共享契约变化。 | — | 任一参与仓库未完成契约对齐和集成验证时，不得单独声称整体可发布。一体仓内前后端同历史时，不把同仓多项目根写成跨仓库契约变更。 |
