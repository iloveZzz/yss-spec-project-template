...
| 研发管理仓库 | 承载 Spec、OpenAPI、架构、Ticket、验证、发布和复盘等研发管理资产的仓库。 | — | 不等同于前端 / 后端代码 monorepo。一体仓只表示与实现项目同居同一 Git 历史，不把两种角色合成一个术语。 |
| 实现仓库 | 承载前端、后端或其他运行时代码及其 Git、CI、MR / PR、测试命令和发布流水线的仓库。 | — | 不要把实现仓库的源码所有权混入研发管理角色；一体仓中源码仍按实现项目登记。 |
| 分仓接入 | 研发管理仓库与前端、后端实现仓库各有独立 Git 历史，靠实现仓库登记和跨仓库切片绑定。 | — | 默认拓扑。不是把源码复制进模板源仓库。不要与一体仓混用。 |
