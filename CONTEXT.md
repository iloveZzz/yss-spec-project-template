...
本文档是项目的业务词汇表，只记录稳定业务语言和协作概念，不记录实现细节、计划草稿或架构说明。
...
| Agent | 执行特定工作流步骤的 AI 协作者。 | — | 不要与生物人审查者、数字人角色或 Ticket 状态混用。运行时实例（Cursor Agent、Codex、Grok Bot 等）是 Agent 的承载，不是角色本身。 |
...
| 用户决定 | 提问者或其明确指定的生物人负责人针对已展示资产、版本及范围作出的可追溯回复。 | — | 数字人审查不能代答；原始回复、指定依据及批准范围按用户决定协议记录。 |
...
| 商业约束 | Plan 与发布阶段需要确认的商业边界、交付承诺、发布窗口和商业非目标。 | — | 由产品经理负责数字人侧的整理与协调；不等同于独立的“商务”数字人角色。 |
...
| Slice Implementation Contract | 阶段 7 进入实现前，由 `yss-implementation-contract-compiler` 根据冻结资产和垂直切片编译、再由生命周期编排器批准的实现合同，新 v3 将上游依据、实施范围、冻结 Skill、工作单元及验证要求单点保存，仅展开适用专项约束；v2 保留历史读取。 | — | 实现合同编译器 只能生成草案，不能自行批准、设置 `ready-for-agent` 或宣布完成；Backend 子合同仍保留额外强约束。 |
...
| `review-ready` | 模板维护显式选择独立审查时的就绪状态；旧候选协议要求 candidate 核验、首次完整验证和摘要冻结。 | — | Reviewer 必须消费同一候选摘要；候选字节变化后旧审查失效。 |
| `release-ready` | 模板维护已完成维护者自检和发布前完整 Fresh Verification；显式选择独立审查时还须闭合该审查。 | — | 仍不替代 `gate.release-ready` 所需的生物人发布裁决。 |
| 实现验证命令 | 前端测试 / 构建优先 `pnpm`，后端校验 / 测试 / 编译优先 `./mvnw`。 | — | 不要默认 `npm`、`yarn` 或裸 `mvn`；仓库确实缺少 pnpm 或 Maven Wrapper 时记受控例外。 |
| 脚手架架构选择 | 新建后端工程在工程基线中对 `domain-driven` 与 `layered-mvc` 的结构化选择；Agent 提供证据化推荐，用户确认后由生命周期批准并供脚手架合同消费。 | — | 不是新门禁，也不是生成器内的交互参数；缺失、未确认或摘要漂移时阻断生成。 |
...
| 战略交接快照包 | 将批准的战略资产及证据冻结为可离线验证的完整目录或 ZIP，并提供规则和场景的派生索引。 | — | 源规则由战略方维护；包内快照与索引不成为目标仓词汇权威。 |
| 后端交付包 | 将后端当前切片的战略基线、冻结接口、批准合同及构建、部署和验证证据绑定为可独立接收的交付快照。 | — | 验包通过不代表真实服务当前可用，也不代表前端已获准实现。 |
| 前端联合接收 | 前端对战略交接快照与后端交付包进行版本、词汇、场景、接口和运行条件核验的过程。 | — | 输入就绪允许准备实现计划和合同；写代码仍需批准且当前的 Slice Implementation Contract。 |
| 技术设计合同 | 按已确认架构统一记录共同输入、规则与场景承接、对应设计及审查证据的合同。 | — | 技术设计由通用入口组织，领域建模与传统分层是可选分支，生命周期负责批准。 |
| 战术承接记录 | 将源规则和关键场景关联到所选架构的设计落点、测试 seam、处置结论及依赖切片的结构化记录。 | — | 延期登记不等于依赖切片可实现；冲突回交战略方，未知依赖扩大阻断。 |
...
| 聚合门禁 | 汇总适用自动检查和独立专业审查后，对当前资产与范围作一次批准或验收；权威定义见生命周期注册表。 | — | 内部检查失败仍阻断；有效批准只在相关依据、范围或授权变化时失效。 |
| 授权延续 | 原始用户批准明确覆盖当前边界、资产范围和决定依据时，经独立审查与当前验证继续推进。 | — | 不产生新的用户回复；未知或实质变化、撤回和外部强制审批缺失不能延续。证据格式见用户决定协议。 |
| 内部检查 | 以 `check.*` 登记的自动前置核验、计算状态或独立专业审查；作为聚合门禁输入，不单独请求用户批准。 | — | 不等于省略检查或允许起草者自审。 |
| 交付验收 | 对当前交付候选的独立审查、实际验证和回滚证据作出验收结论。 | — | 不授予实际合并、推送或发布权限；这些动作另按授权范围执行。 |
