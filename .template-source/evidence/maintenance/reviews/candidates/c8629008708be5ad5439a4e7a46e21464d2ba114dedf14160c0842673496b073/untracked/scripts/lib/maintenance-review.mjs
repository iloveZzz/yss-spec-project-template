...
function validateCandidateManifest(manifest, record, baseDir) {
  ...
  if (manifest.review_mode === "worktree") {
    ...
    resolvePortableFile(manifest.tracked_diff_ref, baseDir, "tracked_diff_ref");
    const actualDigest = createHash("sha256").update(readFileSync(streamPath)).digest("hex");
...
function validateTaskPackageBinding(taskPackage, record) {
  ensure(taskPackage?.schema_version === 1, "审查任务包 schema_version 必须为 1");
