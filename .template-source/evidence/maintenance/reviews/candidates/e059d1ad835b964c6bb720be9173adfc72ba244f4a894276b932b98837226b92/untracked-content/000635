...
主控不在本分支兼任业务角色；跨仓或责任冲突时仍按 `dual_hat_split_when`（`cross-repo-load` 或 `responsibility-conflict`）分体。
...
- `project-instance`：`duplicate-and-bind-repo-path`。复制 YAML 的 `title` / `description` / `core_skills`，写入本仓库路径，选择 `runtime_id`。步骤见 `docs/templates/digital-human-runtime-profile-template.md`。外部 `create-yss-harness-design` 尚未接管此步骤。
