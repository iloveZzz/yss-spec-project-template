...
import os,sys,time,json,hashlib,subprocess,signal,resource,re
...
def run(label,cwd,args,*,repeat=3,timeout=90,trace=False,extra_env=None):
 ...
 for i in range(repeat):
  ...
  p=subprocess.Popen(["/usr/bin/time", "-l", *args],cwd=cwd,env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,start_new_session=True)
  timedout=False
  ...
  rss=re.search(rb'(\d+)\s+maximum resident set size',stderr)
  row['peak_rss_bytes']=int(rss[1]) if rss else None
