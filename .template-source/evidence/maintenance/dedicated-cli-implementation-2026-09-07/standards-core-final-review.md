...
- `candidate_snapshot_ref`: `561d8786cf48b4629d02bde376b44b91b9de4f92`
- `candidate_digest`: `182d5175f1bdebbced528326e46f84323b7c6f12`
- 输入：`standards-core-final-task.json` 与 `core-final-review-candidate.json`；唯一写入本报告。
- 完整 diff：`git diff 5eb824d776dba30b4d39522a0555df87f877501b...561d8786cf48b4629d02bde376b44b91b9de4f92`。
- 提交列表：`7b2c354 feat(cli): add dedicated Harness CLI core and confirmed design`、`3215ea7 fix(cli): preflight recovery boundaries and normalize update paths`、`561d878 fix(cli): isolate install roots and verify dedicated delivery instances`。
- 实际核验 tree、merge-base、提交列表和 diff --check。前两次已读取的未变代码与本次完整增量构成本固定候选；测试代码由 `git show 561d878:<path>` 独立导出。未以活动工作树代替候选。
- 范围：公共核心与薄包生成器 A–E，以及本次新增根执行范围 schema v2、分发验证入口及中文手册修改。来源子模板、真实薄包/tgz 和 GitHub 发布由主控另外验收；这里不声明其通过。
...
实际开始时间：2026-09-07T02:54:09.344813Z。核心全部文件从固定提交导出到独立临时目录，执行后清理，未改变候选。

| 命令 / 场景 | 退出码 | 结果 |
|---|---|---|
| git diff --check 5eb824d...561d878 | 0 | 无空白错误 |
| pnpm --dir <固定提交导出目录>/.template-source/cli-core test | 0 | 17 tests、17 pass、0 fail、15925.393417ms |
...
| 上游核心执行记录 | 0（任务包） | core-final-test.txt；不使用任务包复制的旧时间替代本次独立运行时间 |
...
```json
{
  ...
  "task_id": "dedicated-cli-core-standards-fix-final",
  ...
  "changed_files": [".template-source/evidence/maintenance/dedicated-cli-implementation-2026-09-07/standards-core-final-review.md"],
  "evidence_files": [".template-source/evidence/maintenance/dedicated-cli-implementation-2026-09-07/standards-core-final-review.md"],
  "new_impacts": [],
  "drift": [],
  "seam_deferred": [],
