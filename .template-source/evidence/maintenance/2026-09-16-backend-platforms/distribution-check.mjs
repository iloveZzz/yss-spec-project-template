...
try {
 for(const side of ['backend','frontend','design']) {
  ...
  for(const ref of ['docs/engineering/backend-platforms.json','scripts/lib/backend-platform.mjs','docs/process/schemas/project-scaffold-contract.schema.json'])assert.ok(snapshot.files[ref],`${side} missing ${ref}`);
