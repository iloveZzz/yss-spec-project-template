...
|---|---:|---:|
| 本体 | 220 | 198 |
| yss-harness-design-agent | 137 | 132 |
| yss-harness-dev-agent | 181 | 159 |
| create-yss-spec | 215 | 193 |
| create-yss-strategic-design | 116 | 111 |
| create-yss-harness-dev | 180 | 158 |
| yss-harness-backend-agent | 177 | 155 |
| yss-harness-frontend-agent | 158 | 136 |
| create-yss-harness-backend | 3 | 3 |
| create-yss-harness-frontend | 3 | 3 |
...
| 命令/路径 | 基线耗时 | 样本数 | 退出码 |
|---|---:|---:|---|
| 本体身份识别 | 0.034 秒 | 3 | 0 / 0 / 0 |
| 本体词汇校验 CLI（含扫描） | 0.046 秒 | 3 | 0 / 0 / 0 |
| 本体路由查询 CLI | 0.087 秒 | 3 | 0 / 0 / 0 |
| 本体技能投影 check | 0.096 秒 | 3 | 0 / 0 / 0 |
| 本体技能锁 check | 0.084 秒 | 3 | 0 / 0 / 0 |
| Plan 入口场景 | 1.665 秒 | 1 | 0 |
| 生命周期转换场景 | 1.727 秒 | 1 | 0 |
| 战略交接包场景 | 20.827 秒 | 1 | 0 |
| 前后端联合接收场景 | 32.023 秒 | 1 | 0 |
| 后端 CLI 包验证 | 0.212 秒 | 3 | 0 / 0 / 0 |
| 前端 CLI 包验证 | 0.219 秒 | 3 | 0 / 0 / 0 |
| 后端 CLI init | 137.017 秒 | 1 | 0 |
| 前端 CLI init | 102.735 秒 | 1 | 0 |
| 综合 CLI 本地源同步 | 3.332 秒 | 1 | 0 |
| 战略 CLI 本地源同步 | 0.664 秒 | 1 | 0 |
| 开发 CLI 本地源同步 | 3.102 秒 | 1 | 0 |

当前共记录105次命令运行，含追踪和失败输入；不是105份不同实现。完整逐次命令、时间、CPU、退出码、输出和追踪见[timings.csv](measurements/timings.csv)、[runs.jsonl](measurements/runs.jsonl)和[trace-summary.json](measurements/trace-summary.json)。

有效的合成Plan入口直接API调用测了15次，每次 **180.3–193.7 毫秒**。5次调用的追踪里，主进程等待Node对账CLI约542ms、等待Python用户决定schema约335ms；对账CLI内部又启动Python。读取和摘要成本与这些子进程启动成本不是同一量级。
...
使用真实 context reconciliation schema，对同样的20份合成输入（10合法、10非法）作3组对照，逐条启动20个Python进程的中位数为 **1.348 秒**，一次进程批量校验为 **0.067 秒**；错误列表完全一致。该实验只证明启动摊销可行，未实现生产替代，也未覆盖文件新鲜度、用户回复来源、全部schema及跨schema引用。
