...
export function preserved(ref) { return ['README.md', 'CONTEXT.md', 'DESIGN.md', '.template-spec/agents/issue-tracker.md'].includes(ref); }
export function userOwned(ref, family) {
  ...
  if (family.side === 'design') return !(/^(?:\.(?:agents|claude|codex|cursor|pi|qoder|trae)\/skills\/|scripts\/|\.template-spec\/(?:agents\/|process\/|templates\/|user-guide\/|architecture\/templates\/))/.test(ref)
