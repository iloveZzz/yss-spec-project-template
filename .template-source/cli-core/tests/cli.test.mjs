...
function fixture(t, side = "backend") {
  ...
  const files = {
    ...
    ".template-spec/process/rule.md": "rule v1\n",
...
test("旧治理目录仅在显式迁移时转入新目录，保留项目 tracker 配置", (t) => {
  const f = fixture(t, "backend");
  f.bundle({".template-spec/agents/issue-tracker.md":"default\n"});
  fs.mkdirSync(f.target);
  assert.equal(f.run("attach", "--apply").status, 0);
  const metadataPath = path.join(f.target, f.family.metadataFile);
  const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
  for (const suffix of ["process/harness-profile.yaml", "process/rule.md", "agents/issue-tracker.md"]) {
    const to = `.template-spec/${suffix}`, from = `docs/${suffix}`;
    fs.mkdirSync(path.dirname(path.join(f.target, from)), {recursive:true});
    fs.renameSync(path.join(f.target, to), path.join(f.target, from));
    metadata.managedFiles[from] = metadata.managedFiles[to];
    delete metadata.managedFiles[to];
  }
  fs.writeFileSync(path.join(f.target, "docs/agents/issue-tracker.md"), "custom-project-tracker\n");
  metadata.baselineDigest = hash(JSON.stringify(metadata.managedFiles));
  fs.writeFileSync(metadataPath, json(metadata));
  const blocked = f.run("sync", "--apply", "--force");
  assert.equal(blocked.status, 1);
  const preview = f.run("sync", "--migrate-layout", "--plan");
  assert.equal(preview.status, 0, preview.stderr);
  assert.equal(preview.data.migrationRequired, true);
  assert.equal(f.run("sync", "--migrate-layout", "--apply").status, 0);
  assert.equal(fs.readFileSync(path.join(f.target, ".template-spec/agents/issue-tracker.md"), "utf8"), "custom-project-tracker\n");
  assert.equal(fs.existsSync(path.join(f.target, "docs/agents/issue-tracker.md")), false);
  assert.equal(f.run("sync", "--apply").status, 0);
});
test("显式迁移不覆盖已修改的旧治理文件", (t) => {
  const f = fixture(t, "frontend");
  fs.mkdirSync(f.target);
  assert.equal(f.run("attach", "--apply").status, 0);
  const metadataPath = path.join(f.target, f.family.metadataFile);
  const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
  const old = "docs/process/rule.md", current = ".template-spec/process/rule.md";
  fs.mkdirSync(path.dirname(path.join(f.target, old)), {recursive:true});
  fs.renameSync(path.join(f.target, current), path.join(f.target, old));
  metadata.managedFiles[old] = metadata.managedFiles[current];
  delete metadata.managedFiles[current];
  metadata.baselineDigest = hash(JSON.stringify(metadata.managedFiles));
  fs.writeFileSync(metadataPath, json(metadata));
  fs.writeFileSync(path.join(f.target, old), "user changes\n");
  const result = f.run("sync", "--migrate-layout", "--apply", "--force");
  assert.equal(result.status, 1);
  assert.equal(result.data.code, "LAYOUT");
  assert.equal(fs.readFileSync(path.join(f.target, old), "utf8"), "user changes\n");
});
...
test('通用命令只读、README 保留、退出分发必须显式 prune', t => {
  for (const side of ['design', 'backend', 'frontend']) {
    ...
    f.bundle({'.template-spec/process/rule.md': null});
    assert.equal(f.run('sync', '--apply').status, 0);
    assert.ok(fs.existsSync(path.join(f.target, '.template-spec/process/rule.md')));
    const prunePlan=f.run('sync','--plan','--prune');assert.equal(prunePlan.status,0);assert.deepEqual(prunePlan.data.pruned,[]);assert.ok(prunePlan.data.prunable.includes('.template-spec/process/rule.md'));
    assert.equal(f.run('sync', '--apply', '--prune').status, 0);
    assert.equal(fs.existsSync(path.join(f.target, '.template-spec/process/rule.md')), false);
  }
});
function tree(root) {
...
test("sync 三方规则：本地保留、模板升级、新增、删除和冲突整次暂停", (t) => {
  ...
  put(f.target, ".template-spec/process/rule.md", "local edit\n");
  const before = tree(f.target);
  let r = f.run("sync");
  assert.equal(r.status, 0, r.stderr);
  assert.ok(
    r.data.changes.some(
      (x) => x.path === ".template-spec/process/rule.md" && x.action === "preserve",
    ),
  );
  assert.deepEqual(tree(f.target), before);
  f.bundle({
    ".template-spec/process/rule.md": "upstream v2\n",
    "docs/new.md": "new\n",
    "scripts/check": null,
  });
  r = f.run("sync", "--apply");
  ...
  assert.equal(r.status, 0, r.stderr);
  assert.equal(
    fs.readFileSync(path.join(f.target, ".template-spec/process/rule.md"), "utf8"),
    "upstream v2\n",
  );
...
test("init dry-run 无写入；非空目录、未知基线、目录碰撞、嵌套仓库和快照越界均拒绝", (t) => {
  ...
  for (const kind of ["dir", "link", "repo", "snapshot"]) {
    ...
    if (kind === "repo") put(x.target, ".template-spec/.git", "gitdir: elsewhere");
...
test("写入失败自动恢复内容与 metadata，备份失败不应用计划", (t) => {
  for (const boundary of ["write", "backup"]) {
    const f = fixture(t);
    assert.equal(f.run("init").status, 0);
    f.bundle({ "docs/new.md": "new", ".template-spec/process/rule.md": "v2" });
    ...
    const r = injected(
      f,
      `const original=fs.renameSync;let failed=false;fs.renameSync=(a,b)=>{if(!failed&&String(b).includes(${JSON.stringify(boundary === "write" ? "/.template-spec/process/rule.md" : "/backup/")})){failed=true;throw new Error('injected disk error')}return original(a,b)};`,
    );
...
test("中断恢复前拒绝旧身份、嵌套仓库、声明gitlink及损坏备份，整次零写入", (t) => {
  for (const kind of ["legacy", "repo", "gitlink", "backup"]) {
    ...
    const r = injected(
      f,
      `const original=fs.renameSync;fs.renameSync=(a,b)=>{original(a,b);if(String(b).endsWith('/.template-spec/process/rule.md'))process.kill(process.pid,'SIGKILL')};`,
    );
    assert.equal(r.signal, "SIGKILL");
    if (kind === "legacy")
      put(
    ...
    if (kind === "repo") put(f.target, ".template-spec/.git", "gitdir: elsewhere");
    if (kind === "gitlink")
      put(
        f.target,
        ".gitmodules",
...
test('同步与接入保留已有业务词汇表，即使模板同时更新且传入 force', t => {
  for (const side of ['design', 'backend', 'frontend']) {
    ...
    f.bundle({ 'CONTEXT.md': '# 新模板流程词汇\n', '.template-spec/process/rule.md': 'rule v2\n' });
    const before = tree(f.target);
    ...
    assert.equal(fs.readFileSync(path.join(f.target, '.template-spec/process/rule.md'), 'utf8'), 'rule v2\n');
...
test("WAL 兼容旧 progress 恢复，忽略未完成尾部且拒绝完整损坏记录", t => {
  for (const kind of ["legacy", "torn-tail", "corrupt-record"]) {
    ...
    if(kind !== "corrupt-record") {
      assert.equal(recovered.data.status,"recovered");
      assert.equal(fs.existsSync(path.join(f.target,"docs/new.md")),false);
      assert.equal(fs.readFileSync(path.join(f.target,".template-spec/process/rule.md"),"utf8"),"rule v1\n");
    }
  }
});
...
test("持久化模型：目录项未落盘时恢复接受 before/after 混合状态", t => {
  const f=fixture(t);assert.equal(f.run("init").status,0);
  const old=fs.readFileSync(path.join(f.target,".template-spec/process/rule.md"));
  f.bundle({"docs/new.md":"new",".template-spec/process/rule.md":"v2"});
  const r=injected(f,`const original=fs.renameSync;fs.renameSync=(a,b)=>{original(a,b);if(String(b).endsWith('/.template-spec/process/rule.md'))process.kill(process.pid,'SIGKILL')};`);
  assert.equal(r.signal,"SIGKILL");
  // Model the lost unflushed renames independently; this is not a physical power cut.
  fs.writeFileSync(path.join(f.target,".template-spec/process/rule.md"),old);
  if (fs.existsSync(path.join(f.target,"docs/new.md"))) fs.unlinkSync(path.join(f.target,"docs/new.md"));
  const recovered=f.run("sync","--apply");
  assert.equal(recovered.status,0,recovered.stderr);
  assert.deepEqual(fs.readFileSync(path.join(f.target,".template-spec/process/rule.md")),old);
});
...
test("持久化顺序：WAL 与文件 fsync 先于 rename，目录屏障先于 metadata", t => {
  ...
  for(const ref of ['docs/new.md','.template-spec/process/rule.md',f.family.metadataFile]) {
...
test("持久化边界矩阵：异常与进程中断都保留可证明的恢复状态", async t => {
  for (const boundary of ["backup", "wal-write", "wal-fsync", "target-fsync", "rename", "directory", "commit"]) {
    for (const fault of ["throw", "kill"]) await t.test(`${boundary}/${fault}`, () => {
      const f = fixture(t); assert.equal(f.run("init").status, 0);
      const before = tree(f.target); delete before[".yss-harness-state"];
      f.bundle({ "docs/new/nested.md": "new", ".template-spec/process/rule.md": "v2" });
      ...
      if (fault === "kill") {
        ...
        if (boundary === "commit") {
          assert.equal(fs.readFileSync(path.join(f.target, ".template-spec/process/rule.md"), "utf8"), "v2");
          return;
        }
        assert.equal(recovered.data.status, "recovered");
...
test('三家族 .gitignore 区块保留用户字节，损坏标记阻断，prune 不删修改文件', t => {
 for(const side of ['design','backend','frontend']) {
  ...
  put(f.target,'.template-spec/process/rule.md','local');f.bundle({'.template-spec/process/rule.md':null});
  const r=f.run('sync','--apply','--prune',...(side==='design'?[]:['--force']));assert.equal(r.status,0,r.stderr);
  assert.equal(fs.readFileSync(path.join(f.target,'.template-spec/process/rule.md'),'utf8'),'local');
...
test('三家族手册退出分发仅由 prune 删除未修改 baseline', t => {
 for(const side of ['design','backend','frontend']) {
  const f=fixture(t,side);f.bundle({'.template-spec/user-guide/retired.md':'retired\n','.template-spec/user-guide/retained.md':'retained\n'});
  assert.equal(f.run('init').status,0);
  fs.appendFileSync(path.join(f.target,'.template-spec/user-guide/retained.md'),'用户修改\n');
  f.bundle({'.template-spec/user-guide/retired.md':null,'.template-spec/user-guide/retained.md':null});
  const normal=f.run('sync','--apply');assert.equal(normal.status,0,normal.stderr);
  assert.ok(fs.existsSync(path.join(f.target,'.template-spec/user-guide/retired.md')));
  const plan=f.run('sync','--plan','--prune');assert.equal(plan.status,0,plan.stderr);
  assert.ok(plan.data.prunable.includes('.template-spec/user-guide/retired.md'));
  assert.ok(plan.data.retainedRemoved.includes('.template-spec/user-guide/retained.md'));
  const applied=f.run('sync','--apply','--prune');assert.equal(applied.status,0,applied.stderr);
  assert.ok(!fs.existsSync(path.join(f.target,'.template-spec/user-guide/retired.md')));
  assert.match(fs.readFileSync(path.join(f.target,'.template-spec/user-guide/retained.md'),'utf8'),/用户修改/);
 }
});
...
test('三家族参数、异族、硬链接和嵌套仓库不能经新命令绕过', t => {
 for(const side of ['design','backend','frontend']) {
  ...
  const rule=path.join(f.target,'.template-spec/process/rule.md');fs.linkSync(rule,path.join(f.target,'linked'));
  assert.equal(f.run('diff').status,1);assert.equal(f.run('doctor').status,1);fs.unlinkSync(path.join(f.target,'linked'));
  put(f.target,'.template-spec/.git/config','');assert.equal(f.run('diff').status,1);assert.equal(f.run('doctor').status,1);
 }
});
...
test('战略 v1 缺少证据不迁移，合法来源仅在 apply 转换，未知 mode 不授予 prune',t=>{
 ...
 f.bundle({'.template-spec/process/rule.md':null});const r=f.run('sync','--prune','--apply');assert.equal(r.status,0,r.stderr);
 assert.ok(fs.existsSync(path.join(f.target,'.template-spec/process/rule.md')));assert.equal(JSON.parse(fs.readFileSync(metadata)).metadataSchemaVersion,2);assert.equal(fs.readFileSync(path.join(f.target,'scripts/instantiate-harness'),'utf8'),'legacy script');
 assert.equal(f.run('sync','--prune','--apply').status,0);assert.ok(fs.existsSync(path.join(f.target,'.template-spec/process/rule.md')));
});
...
test('三家族真实写入、中断、备份损坏、并发修改及校验失败均保持恢复边界',t=>{
 for(const side of ['design','backend','frontend']) for(const failure of ['write','kill','backup-corrupt','concurrent','verify']) {
  ...
  else code=`const original=fs.renameSync;fs.renameSync=(a,b)=>{original(a,b);if(String(b).endsWith('/.template-spec/process/rule.md')){${failure==='write'?"throw new Error('injected write failure')":"process.kill(process.pid,'SIGKILL')"}}};`;
  const result=injected(f,code);
  if(['write','verify'].includes(failure)) {assert.equal(result.status,1,result.stderr);assert.deepEqual(fs.readFileSync(metadata),before);assert.equal(fs.readFileSync(path.join(f.target,'.template-spec/process/rule.md'),'utf8'),'rule v1\n');continue;}
  assert.equal(result.signal,'SIGKILL',result.stderr);
  ...
  if(failure==='backup-corrupt') {const index=journal.operations.findIndex(op=>op.path==='.template-spec/process/rule.md');fs.writeFileSync(path.join(base,'backup',String(index)),'corrupt');}
  if(failure==='concurrent') fs.writeFileSync(path.join(f.target,'.template-spec/process/rule.md'),'user subsequent edit');
...
test('tracker remains project-owned on sync and force; new init enables stage tracking', t => {
  ...
  x.bundle({ '.template-spec/agents/issue-tracker.md': old });
