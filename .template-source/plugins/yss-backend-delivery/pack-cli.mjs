...
export function packCli(cliRoot, pin) {
  ...
  let preview;
  try {
    preview = spawnSync(process.execPath, [path.join(root, 'bin/create-yss-spec.js'), '--project-name', 'Plugin binding baseline',
      '--business-domain', 'Template tooling', '--team-size', '1', '--issue-tracker', 'github', '--no-example-docs',
      '--target-dir', path.join(temp, 'governance'), '--dry-run'], { cwd: temp, input: '', encoding: 'utf8', timeout: 60000, maxBuffer: 16 * 1024 * 1024 });
