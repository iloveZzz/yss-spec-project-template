...
- 每个生命周期阶段完成后必须执行 Git checkpoint 判断：列出阶段产物、Issue 同步状态、排除无关脏文件，并在用户已授权时按范围提交和推送；不得连续推进多个阶段却让 PRD、设计、OpenAPI、架构、评审或 Issue 产物长期只停留在本地工作区或未同步到选定 issue tracker。
...
当产品代码放入本仓库时，默认使用轻量 monorepo 目录：
...
Comet open / design / build 或任何脚手架生成步骤，在写入业务代码、生成后端 / 前端工程骨架或创建实现目录前，必须先检查并记录实现位置；如果已有 proposal、design、build entry review 或实施计划已经记录过实现位置，后续阶段复用该记录，不重复询问：

- 默认后端代码进入 `apps/backend/`，默认前端代码进入 `apps/frontend/`。
- 发布说明进入 `docs/releases/`，实施方案、上线记录和回滚方案进入 `docs/implementation/`。
- 如需新建或改用其他目录（例如已有工程目录、外部 worktree 或临时 POC 目录），必须获得用户明确确认，并在 proposal、design、build entry review 或实施计划中记录偏离原因。
- 已存在的历史工程目录不得被强制搬迁；继续使用时必须把它记录为当前 change 的实现位置，并说明是否计划后续迁移到默认 `apps/` 结构。
