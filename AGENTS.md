...
13 个工作单元是 Harness 工程的对外沟通和试点说明表达层，不替代 9 个主阶段 / 21 个门禁。执行、审查、追踪和裁剪仍以 `docs/process/lifecycle-artifact-map.md` 为权威；13 个工作单元到执行体系的映射见 `docs/process/harness-work-unit-map.md`。
...
13 个工作单元可用于向业务方解释端到端链路；Agent 实际执行时必须回到 9 个主阶段 / 21 个门禁、流程裁剪指南和对应模板。
