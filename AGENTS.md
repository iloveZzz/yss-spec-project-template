...
```
Spec 定义要做什么，YSS 定义工程边界；Matt skills 让澄清、拆分、实现、审查和验证保持轻量可追溯。OpenAPI 定义前后端契约。
```
...
能力说明见 `docs/process/MATT-POCOCK-ENGINEERING-SKILLS.md`。本仓库已引入项目级 `mapstruct` 和 `lombok` skills，来源为 `claude-dev-suite/claude-dev-suite`；当后端实现涉及 POJO 样板代码、DTO / VO / PO / Domain Model 对象转换、`Convertor`、`Mapper`、`@Mapper`、`@Mapping`、`@MappingTarget`、`@Data`、`@Getter`、`@Setter`、`@Builder`、`@RequiredArgsConstructor` 或 `@Slf4j` 时，必须加载对应 skill。常用链路：
...
流程状态必须同步到选定 issue tracker：Spec、需求冻结、OpenAPI Freeze、垂直切片 Ticket、阶段完成、验证结果、阻塞项和人工审查点都必须更新到对应 Ticket / 里程碑 / 评论中，不得只停留在本地文档。
...
若上游 Matt / YSS skill、外部插件或模板本身使用英文标题、英文说明或英文 checklist，落地到本仓库的持久化文档时必须转换为中文正文和中文章节标题；英文模板只能作为结构参考，不得原样输出为面向业务、产品、架构、实施、审查或复盘的交付文档。例外仅限协议字段、metadata、代码标识和用户明确要求英文的文档。
...
YSS 全生命周期允许使用 subagent 提效，但必须遵守 `docs/process/subagent-collaboration.md`：subagent 只能承担证据收集、草案产出、局部实现、独立审查和验证执行；主控 Agent 负责生命周期阶段判定、需求冻结、OpenAPI Freeze、架构放行、Issue 同步、Git checkpoint 和完成 / 可合并 / 可发布结论。派发 subagent 前必须形成任务包或等价记录，模板见 `docs/templates/subagent-task-package-template.md`。
...
- 任务开始前必须先参考 `docs/process/harness-process-tailoring.md` 判断小改动 / 中等变更 / 新模块或高风险变更；裁剪只能减少不相关产物，不能裁剪人工审查、Issue 追踪、Git checkpoint 和 fresh verification。
...
- subagent 不得替代主控 Agent 执行生命周期阶段判定、Spec baseline、requirement freeze、Prototype 用户确认、OpenAPI Freeze、Design / Architecture Review 最终放行、Spec Delta 必要性最终判断、Issue tracker 最终状态、Git checkpoint 范围裁决或“完成 / 可合并 / 可发布”结论。
...
- 进入实现前，必须把系统架构、数据架构、ADR、工程基线、OpenAPI Freeze 结论和风险 / 回滚约束转译为当前切片的 `Build Architecture Checklist`。
- 跨线程、跨仓库、上下文接近上限、原型分支回流或实现仓库交接时，必须使用 `handoff` 或等价交接记录，写清来源资产、当前阶段、未决问题、验证命令和下一步责任人。
- 每个垂直切片完成时必须回勾 `Build Architecture Checklist`：已落实项给出代码、测试或文档证据；延期项说明原因、风险、补齐切片和是否允许继续；漂移项必须触发 Architecture Re-check；违反项必须暂停并记录替代方案并回到设计审查或架构修正。
- 涉及 Repository / Gateway / 持久化边界、权限 / 认证 / 授权、审计日志、SQL / DDL / 数据库迁移、文件上传下载 / 临时 URL、版本快照 / 元数据 / 血缘 / 查询索引、跨模块接口、部署、回滚或运维约束时，必须在当前切片的实现记录中写明架构对齐状态、验证证据、责任人和需要补齐的落点。
...
- 发布后或阶段性完成后必须做复盘判断；若发生架构返工、verify 返工、IMPORTANT / CRITICAL review findings 或人工确认延期，必须落中文复盘记录，并把流程修订回写到 `AGENTS.md`、`docs/process/lifecycle-artifact-map.md`、ADR 或模板中。
...
3. 从零后端骨架生成后，必须用 `yss-backend-scaffold-parent` 做工程基线检查；不得把脚手架 sample 代码当作业务实现交付。
   - 后端工程构建、测试、运行、OpenAPI 生成和 CI 验证命令必须优先使用项目根目录 `./mvnw`；新脚手架必须提交 `mvnw`、`mvnw.cmd` 和 `.mvn/` wrapper 资产。实现计划、Ticket、Review、Release、README 或 CI 中出现裸 `mvn ...` 时，除非记录了既有仓库无法使用 wrapper 的受控例外，否则视为规范偏离并要求改为 `./mvnw ...`。
...
5. 每个垂直切片在写业务代码前，按影响面加载专项规范：
  ...
  - Repository / PO / Convertor / GatewayImpl：`yss-repository` + `mapstruct` + `lombok`，需要 MyBatis 细节时接 `yss-mybatis` / `yss-backend-scaffold-infrastructure`。
  - Controller / DTO / VO / Web Convertor：`yss-web-controller`、`yss-dto`、`yss-backend-scaffold-web` + `mapstruct` + `lombok`。
  - 前端页面、组件、API 接入、YSS UI：按 `yss-router` 选择 `yss-page-module-development`、`yss-components`、`api-integration` 等最小集合。
6. 后端 POJO 类默认使用 `lombok` 处理样板代码；DTO / VO / CMD / Query / PO / Domain Model 等对象若需要 getter/setter、构造器、builder 或日志字段，必须优先使用 Lombok 注解并遵守 `lombok` skill 的反模式约束。禁止为了省事手写成片 getter/setter、constructor、builder、logger；受控例外必须写入实现合同和 review 记录。
7. 后端对象转换默认使用 `mapstruct`；`PO <-> Domain Model`、`Domain/Application Result -> VO/DTO`、`CMD/Query -> Command/Input Model`、集合转换、更新映射和枚举转换必须优先通过 MapStruct `Convertor` / `Mapper` 落地。禁止使用 `BeanUtils.copyProperties`、反射式通用拷贝、Controller / Application 内大段手写 mapping 或重复字段赋值；受控例外必须说明原因、范围、测试证据和补齐落点。
8. Java 后端实现和审查必须同时遵守 `alibaba-java-code-style`；命名、异常、日志、集合、单元测试、ORM/MyBatis、Maven、MapStruct / Lombok 注解处理器配置和安全相关强制项作为 review blocker 处理。
9. 实现计划和 review 记录必须列出本切片实际使用的 YSS / MapStruct / Lombok / Alibaba 规范输入、验证命令和未覆盖原因；不得用“符合 YSS”笼统替代具体 skill 证据。
10. 任何后端垂直切片在写业务代码前，必须先形成 `Backend Slice Implementation Contract` 或等价记录；缺少 `required_skills`、`allowed_write_paths`、`forbidden_patterns`、`expected_evidence_files`、`seam_deferred`、`verification_commands` 任一项时，Agent 必须暂停并记录替代方案并回到 implementation routing / 切片 Ticket 补齐。
11. 后端切片完成后必须用代码、测试或文档证据回勾 `Backend Slice Implementation Contract` 和 `Build Architecture Checklist`；未回勾的 `required_skills` 视为 `violation`，不能声明完成 / 可合并。
12. 以下情况默认作为 YSS 后端实现 review blocker，除非在合同和 review 记录中给出 `yss-dto` / `yss-web-controller` / `mapstruct` / `lombok` 等专项 skill 依据、例外原因和补齐落点；注意 `CMD` / `Query` / `VO` / `SingleResult` / `MultiResult` / `PageResult` 本身是 `yss-dto` 规范产物，阻断的是私自发明、内嵌、混用或绕过既有 DTO 体系：
  ...
  - `PO / Domain Model / DTO / VO / CMD / Query` 等 POJO 成片手写 getter/setter、constructor、builder、logger，未使用 Lombok 或未记录受控例外。
  - `PO <-> Domain Model`、`Domain/Application Result -> VO/DTO`、集合或更新映射使用 `BeanUtils.copyProperties`、反射拷贝或重复手写字段赋值，未使用 MapStruct 或未记录受控例外。
...
- POJO 样板代码优先使用 Lombok，避免成片手写 getter/setter、constructor、builder 和 logger。
- 对象转换必须优先使用 MapStruct，不使用 `BeanUtils.copyProperties`、反射式通用拷贝或大段重复 mapping；确需手写时必须有受控例外、测试证据和 review 记录。
...
| 小需求变更 / 已有功能迭代 | 影响面评估 -> 选择最近可信阶段 -> 更新受影响资产 -> 必要评审 -> TDD / verify | 不重跑机会探索和全量 Spec；若影响 API、状态机、权限、数据模型、跨端协作或高风险行为变化，则从对应阶段延伸并补齐下游门禁 |
...
| 审查点 | 触发时机 | 主审 | 阻断项 |
| --- | --- | --- | --- |
| Spec Review | Spec 完成后 | Product / Domain Review Agent | 用户、痛点、非目标范围、验收标准或风险影响不清 |
| Grilling Review | 新功能 / 较大改动 Spec 前、Architecture Review 前 | Product / Domain / Architecture Agent with `grill-with-docs` | 用户价值、边界、状态、反例、架构约束、验收标准或风险影响经不起质询 |
