...
按领域划分：后端脚手架（`yss-ddd-scaffold-generator` 及 Domain / Application / Infrastructure / Web / Adapter 分层技能）、领域建模（`yss-domain`）、持久层（`yss-repository`、`yss-mybatis`、`yss-db2mybatis`）、Web 契约（`yss-web-controller`、`yss-dto`、`yss-openapi-governance`、`yss-openapi-draft-review`）、前端（`yss-ui`、`yss-components`、`yss-formily`、`yss-hook`、`yss-page-module-development`、`api-integration`）、以及横切组件（`yss-cache`、`yss-exception`、`yss-log`、`yss-userinfo`、`yss-distributed-id`、`yss-security-algorithm`、`yss-resilience4j` 等）。
