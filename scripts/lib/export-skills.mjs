...
const forbidden = ["AGENTS.md", "CONTEXT.md", "docs", "skills-lock.json", ".agents", ".codex", ".cursor", ".pi"];
...
function sanitize(text, relative) {
  ...
  value = value.replace(/(?:\$\{base_project\}\/)?(?:\/path\/to\/)?\.(?:agents|codex|cursor|pi)\/skills\//g, prefix);
  value = value.replace(/\.(?:agents|codex|cursor|pi)\/skills\b/g, "the canonical public skill source");
...
function validate(output, skillNames) {
  ...
  for (const [_relative, absolute] of files(path.join(output, "skills"))) {
    ...
    if (/\.(?:agents|codex|cursor|pi)\/skills/.test(text)) fail("Agent root path in public export: " + absolute);
  }
