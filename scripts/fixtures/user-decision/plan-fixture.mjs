...
export function buildPlanFixture(root) {
  ...
  const review = { schema_version: 1, kind: 'plan-entry-review', gate_id: 'gate.plan-approved', feature_id: 'feature.demo', plan_ref: 'plan.md', context_reconciliation_ref: 'reconciliation.json', basis: ['plan.md', 'CONTEXT.md', 'docs/process/lifecycle-registry.yaml', 'reconciliation.json'].map(ref => ({ ref, digest: decisionDigest(readFileSync(path.join(root, ref))) })), checks: Object.fromEntries(policy.required_checks.map(id => [id, { status: 'passed', evidence_refs: ['plan.md'] }])), open_items: [], impacts: { domain_strategy: false, stage_decision: false }, internal_checks: Object.fromEntries(Object.keys(policy.check_impacts).map(id => [id, { status: 'not-applicable', reason: '本次仅修订已确认范围内的细节，不改变边界、规则或阶段合同', evidence_refs: ['plan.md'] }])) };
