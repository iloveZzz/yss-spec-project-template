...
scripts/hermes init-feature demo-feature --title "Demo Feature" --sprint "Sprint 1"

# 2. 生成当前阶段产物。默认使用内置 stub，适合先跑通流程
scripts/hermes run demo-feature

# 3. 通过 Comet 门禁推进阶段
scripts/hermes advance demo-feature
```

需要更多信息时再展开：

```bash
scripts/hermes status demo-feature
scripts/hermes history demo-feature
scripts/hermes dashboard
scripts/hermes doctor
scripts/hermes collect-metrics
```
...
真实后端通过 CLI contract 接入：执行时会收到 `HERMES_TASK_*` 环境变量，并必须把产物写入 `HERMES_TASK_OUTPUT_PATH`，文件中必须包含 `pipeline: <id>`。

真实执行示例：

```bash
scripts/hermes run demo-feature --backend codex
```

底层入口保留给调试和自动化：`scripts/comet-driver`、`scripts/comet-guard.sh`、`scripts/stage-executor`、`scripts/metrics-collector`。日常使用优先从 `scripts/hermes` 开始。
